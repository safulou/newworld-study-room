...
const MELODY = [
  ...
  ["R", .5], ["C4", .5], ["E4", .5], ["A4", .5], ["B4", 1.5], ["R", .5], ["E4", .5], ["G#4", .5], ["B4", .5], ["C5", 1.5],
  ["R", .5], ["E4", .5], ["E5", .5], ["D#5", .5], ["E5", .5], ["D#5", .5], ["E5", .5], ["B4", .5], ["D5", .5], ["C5", .5], ["A4", 1.5],
  ["R", .5], ["C4", .5], ["E4", .5], ["A4", .5], ["B4", 1.5], ["R", .5], ["E4", .5], ["C5", .5], ["B4", .5], ["A4", 2], ["R", 1],
];
...
export class BackgroundMusic extends EventTarget {
  ...
  async start() {
    ...
    this.master.gain.setTargetAtTime(this.volume / 250, this.context.currentTime, .06);
    this.schedulePhrase();
  ...
  pause() {
    ...
    this.voices.forEach((voice) => {
      try { voice.stop(this.context.currentTime + .12); } catch { /* already stopped */ }
    });
    this.voices.clear();
  ...
  setVolume(value) {
    ...
    if (this.context && this.running) {
      this.master.gain.setTargetAtTime(this.volume / 250, this.context.currentTime, .05);
    }
  }
  ...
  scheduleNote(pitch, start, duration) {
    ...
    envelope.gain.exponentialRampToValueAtTime(.18, start + .018);
    envelope.gain.exponentialRampToValueAtTime(.055, start + Math.min(.22, duration * .5));
    envelope.gain.exponentialRampToValueAtTime(.0001, start + duration + .28);
